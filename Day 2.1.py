...
for i in range(len(rules)):
    passwords.append(rules[i][rules[i].find(":")+2:])
    rules[i] = rules[i][:rules[i].find(":")]

possPass = 0
for i in range(len(rules)):
    min = int(rules[i][:rules[i].find("-")])
    max = int(rules[i][rules[i].find("-")+1:rules[i].find(" ")])
    char = rules[i][rules[i].find(" ")+1:]
    deltaLen = len(passwords[i]) - len(passwords[i].replace(char,""))
    if deltaLen <= max and deltaLen >= min:
        possPass+=1
